...
---
--- This submodule includes sample templates for a variety of advertized services which may be of interest when used with the BonjourLauncher spoon. Each template can be displayed in the Hammerspoon console for reference by typing `help.spoon.BonjourLauncher.recipes.*name*` into the console input field, or added as is to the active templates of the BonjourLauncher by doing the following either in the Hammerspoon console or in your configuration `init.ua` file:
---
---     hs.loadSpoon("BonjourLauncher")
---     table.insert(spoon.BonjourLauncher.templates, spoon.BonjourLauncher.recipes.*name*)
---
...
---                       { type="image", image = hs.image.imageFromName("NSNetwork") },
...
---
---  * See also [BonjourLauncher.recipes.VNC_RealVNC_Alternate](#VNC_RealVNC_Alternate) for an example that can use an alternate launcher for RealVNC clients. Note that you sould use only
---
