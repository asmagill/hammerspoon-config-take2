...
--
--  * spoonName    - string
--  * frameDetails - table containing frameDetails describing where to place canvas within panel (see uitk.manager)
--  * spoonConfig  - table
...
return function(name, frameDetails, config)
    ...
    if config.background then
        ...
        spoonWrapper = uitk.element.container.new(spoonCanvasSize)
        spoonWrapper[1] = {
            id             = "background",
...
-- a change to the uitk wrapper we've created to allow for the background could be inserted for this widget
-- instead.
        ...
        spoonWrapper:frameChangeCallback(function(manager, target)
            ...
            if target == spoonCanvas then
                ...
                -- even without the x and y keys, since uitk imports the table into an NSDictionary, including this key
...
--        NOTE: UITK cannot distinguish between a change in size or a change in location or both -- they all
...
--
--              Hopefully this will all become moot once UITK is in core and canvas is updated to be more
